...
export const criarParticipanteEnsaio = async (req, res) => {
  try {
...
export const listarParticipantesEnsaio = async (req, res) => {
  try {
...
export const atualizarParticipanteEnsaio = async (req, res) => {
  try {
    const { ensaioId, usuarioId } = req.body;
...
export const deletarParticipanteEnsaio = async (req, res) => {
  try {
    await ParticipanteEnsaio.findByIdAndDelete(req.params.id);
    res.status(204).end();
    console.log("Participante excluído com sucesso!");
  } catch (error) {
    console.error("Algo deu errado ao tentar excluir o participante...", error);
  }
};
